...
UNIVERSAL_API HRESULT _stdcall CreateComper(LPVOID* pComper)
{
	...
	LRESULT hResult = CLSIDFromProgID(g_CLSID, &CLSID_CompTestClass);
	if (S_OK != hResult)
	{
	...
	hResult = pUnknown->QueryInterface(IID_IUniversal, pComper);
...
extern "C" HRESULT _stdcall DllGetClassObject(__in REFCLSID rclsid, __in REFIID riid, LPVOID FAR* ppv)
{
	...
	{
		UniversalFactory* pFactory = new UniversalFactory();
		if (NULL == pFactory)
		{
