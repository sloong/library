...
LPCTSTR CLogSystem::FormatFatalMessage( DWORD dwCode, LPCTSTR strErrorText, bool bFormatWinMsg, bool bJustFailedWrite )
{
	if ( FATAL <= m_emLevel )
	{
		LPCTSTR strTmp = NULL;
...
LPCTSTR CLogSystem::FormatErrorMessage( DWORD dwCode, LPCTSTR strErrorText, bool bFormatWinMsg, bool bJustFailedWrite )
{
	if ( ERR <= m_emLevel )
	{
		LPCTSTR strTmp = NULL;
...
LPCTSTR CLogSystem::FormatWarningMessage( DWORD dwCode, LPCTSTR strErrorText, bool bFormatWinMsg, bool bJustFailedWrite )
{
	if ( WARN <= m_emLevel )
	{
		LPCTSTR strTmp = NULL;
...
LPCTSTR CLogSystem::FormatInformationMessage( DWORD dwCode, LPCTSTR strErrorText, bool bFormatWinMsg, bool bJustFailedWrite )
{
	if ( INF <= m_emLevel )
	{
...
void _stdcall CLogSystem::Log(LOGLEVEL emLevel, DWORD dwCode, LPCTSTR strErrorText, bool bFormatWinMsg /* = false */, bool bJustFailedWrite /* = true */)
{
	...
	{
		TCHAR szWinErrText[MAX_STRING+1] = {0};
...
HRESULT CLogSystem::OpenFile()
{
	...
	m_hFileHandle =  CreateFile( m_szFileName, 	GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
bool CLogSystem::IsOpen()
{
	if ( m_emType != LOGTYPE::ONEFILE )
	{
		...
		if( NULL == m_szLastDate || _tcscmp(m_szLastDate, szCurrentDate) != 0)
		{
...
void CLogSystem::Lock()
{
	::EnterCriticalSection(&m_csLock);
}

void CLogSystem::Unlock()
{
...
void CLogSystem::SetConfiguration( LPCTSTR szFileName, LPCTSTR szFilePath, LOGTYPE* pType, LOGLEVEL* pLevel )
{
	...
	if ( szFilePath )
	{
		...
		if(FindFirstFile( szFilePath, &wfd) == INVALID_HANDLE_VALUE && CreateDirectory( szFilePath, NULL) == 0)
		{
			assert(false);
			exit(1);
		}
