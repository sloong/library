...
//Open
void CSloongRecordset::Open(LPCTSTR Source,ISloongConnection & ActiveConn,CursorTypeEnum CursorType,LockTypeEnum LockType,long Options)
{
    m_pRst->Open(_variant_t(Source),ActiveConn.GetConnection().GetInterfacePtr(),CursorType,LockType,Options);
}


void CSloongRecordset::Open(LPCTSTR Source,_ConnectionPtr pConn,CursorTypeEnum CursorType, LockTypeEnum LockType,long Options)
{
...
bool CSloongRecordset::Close() 
{
	if(m_pRst==NULL)
	{
        m_sErrorMessage=_T("Recordset is not available!");
		return true;
	}
	...
	try
	{
		...
		if(state==adStateOpen)
		{
			...
			{
				m_sErrorMessage=_T("Failed to close the Recordset!");
				return false;
			}
...
//GetString
LPCTSTR CSloongRecordset::GetString(LPCTSTR columnName)
{
	try
	{
...
//GetInt
int CSloongRecordset::GetInt(LPCTSTR columnName)
{
	try
	{
...
HRESULT STDMETHODCALLTYPE CSloongRecordset::QueryInterface(REFIID riid, _COM_Outptr_ void **ppvObject)
{
	return S_OK;
}

ULONG STDMETHODCALLTYPE CSloongRecordset::AddRef()
{
	return 0;
}

ULONG STDMETHODCALLTYPE CSloongRecordset::Release()
{
	return 0;
}

void STDMETHODCALLTYPE CSloongRecordset::SetInt(LPCTSTR columnName, int value)
{

}

void STDMETHODCALLTYPE CSloongRecordset::SetString(LPCTSTR columnName, LPCTSTR value)
{
